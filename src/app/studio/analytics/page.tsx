...
export default async function StudioAnalyticsPage() {
  ...
  return (
    <main style={{ maxWidth: 1200, margin: "2rem auto", padding: "1rem", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(180px, 1fr))", gap: "0.8rem", marginBottom: "1rem" }}>
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: "0.8rem" }}>
          <div style={{ color: "#6b7280", fontSize: 13 }}>Total viewers</div>
          <div style={{ fontSize: 24 }}>{summary.totalViewers}</div>
        </div>
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: "0.8rem" }}>
          <div style={{ color: "#6b7280", fontSize: 13 }}>Total opens</div>
          <div style={{ fontSize: 24 }}>{summary.totalOpens}</div>
        </div>
      </div>
